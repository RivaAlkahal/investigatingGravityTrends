...
NUM_ARCS_FOUND=$(find "$CONFIG_DIR" -maxdepth 1 -type f -name 'config_arc_test-*.json' | wc -l | tr -d ' ')
(( NUM_ARCS_FOUND > 0 )) || die "No configs matching '$CONFIG_DIR/config_arc_test-*.json'"
...
launch_arc() {
  ...
  if [[ "$DEBUG" == "1" ]]; then
    printf '+ (dry-run) '
    printf '%q ' "$EXEC_RUN_ARC" --config "$CONFIG_DIR/config_arc_test-${idx}.json" --output-dir "$outdir"
    printf '\n'
    printf '+ (dry-run) '
    ...
    (
      cfg="$CONFIG_DIR/config_arc_test-${idx}.json"
      [[ -f "$cfg" ]] || { say "missing $cfg -skipping"; continue;}
      set -e
      "$EXEC_RUN_ARC" --config "$cfg" \
